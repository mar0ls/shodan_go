...
CATEGORIES = [
    ("CLI", {("type", "searchOptions"), ("type", "searchOutput"), ("func", "parseSearchArgs"), ("func", "formatLine"), ("func", "main")}),
    ("API Client Core", {("type", "Client"), ("func", "NewClient"), "BaseURL"}),
    ("API Models", {("type", "APIInfo"), ("type", "HostLocation"), ("type", "HostHTTP"), ("type", "Meta"), ("type", "Host"), ("type", "FacetCount"), ("type", "SearchResult")}),
    ("API Operations", {("func", "GetAPIInfo"), ("func", "SearchHosts"), ("func", "GetHostByIP")}),
    ("Compatibility Aliases", {("func", "APIInfo"), ("func", "HostSearch"), ("func", "HostLookup"), ("func", "New")}),
]
...
def render_md(package_docs: dict[str, list[str]], blocks: list[dict]):
    ...
    with OUT_FILE.open("w", encoding="utf-8") as f:
        ...
        f.write("info, err := client.GetAPIInfo()\n")
        f.write("if err != nil {\n")
        f.write("    log.Fatal(err)\n")
        f.write("}\n\n")
        f.write("host, err := client.GetHostByIP(\"8.8.8.8\")\n")
        f.write("if err != nil {\n")
        f.write("    log.Fatal(err)\n")
        f.write("}\n")
        ...
        f.write("- Search pagination uses 100 results per page and `--all` consumes additional query credits.\n")
        f.write("- CLI exits early when `SHODAN_API_KEY` is missing.\n")
        f.write("- `--out` path is sanitized and restricted to relative paths in the current working directory.\n\n")
        f.write("- Example (`SHODAN_API_KEY` missing): `SHODAN_API_KEY environment variable not set`.\n")
        f.write("- Example (API non-200): returned as `shodan API error: <status>`.\n\n")
        f.write("---\n\n")
